...
from database.code_repository import insert_code, update_embedding, get_code_by_id, get_embeddings
...
class TestRunner:
    @staticmethod
    def run_test_case(code: str, input_val: str, expected_output: str) -> Tuple[bool, str]:
        """テストケースを実行します。"""
        try:
            # コードをグローバル名前空間で実行
            namespace = {}
...
def find_and_test_similar_code(code: str, test_runner: TestRunner) -> None:
    ...
    if user_input == 'y':
        ...
        for input_val, expected_output in test_cases:
            success, actual = test_runner.run_test_case(code, input_val, expected_output)
